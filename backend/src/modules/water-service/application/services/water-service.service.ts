...
export class WaterServiceService implements IWaterService {
  ...
  async registerService(
    userId: string,
    dto: CreateWaterServiceDto,
  ): Promise<WaterServiceModel> {
    const existingService =
      await this.waterServiceRepository.findByServiceNumber(dto.serviceNumber);

    if (existingService) {
      throw new ConflictException(
        `El Número de Servicio ${dto.serviceNumber} ya se encuentra registrado en el sistema.`,
      );
    }
  ...
  async removeService(id: string, userId: string): Promise<void> {
    ...
    if (!deleted) {
      throw new NotFoundException(
        `No se encontró el servicio con ID ${id} o no tienes permiso para eliminarlo.`,
      );
    }
  }
}
