import { Body, Controller, Delete, Get, Param, Post, Request, UseGuards, Inject } from '@nestjs/common';
...
export class WaterServiceController {
    ...
    @ApiResponse({ status: 201, description: 'Water service successfully hooked to the account.' })
    @ApiResponse({ status: 400, description: 'Bad Request (e.g. invalid Hidalgo municipality).' })
    @ApiResponse({ status: 409, description: 'Conflict. Service Number already registered.' })
    ...
    @Get()
    @Roles(Role.USER, Role.ADMIN)
    @ApiOperation({ summary: 'Get all Water Services registered to the currently logged in Citizen' })
    @ApiResponse({ status: 200, description: 'Returns an array of WaterServiceModel objects.' })
    ...
    @ApiResponse({ status: 200, description: 'Successfully unhooked the service.' })
