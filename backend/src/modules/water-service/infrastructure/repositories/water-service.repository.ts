...
export class WaterServiceRepository implements IWaterServiceRepository {
    ...
    async create(userId: string, data: Partial<WaterServiceModel>): Promise<WaterServiceModel> {
    ...
    async findByServiceNumber(serviceNumber: string): Promise<WaterServiceModel | null> {
        const service = await this.ormRepository.findOne({ where: { serviceNumber } });
