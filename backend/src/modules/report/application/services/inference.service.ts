    ...
    async handleInferenceCron() {
        ...
        try {
            ...
            for (const zone of zones) {
                ...
                const recentRestoredReports = allReports.filter(r =>
                    r.type === ReportType.SUPPLY_RESTORED &&
                    r.createdAt >= fifteenMinutesAgo
                );

                // Threshold: If we have 2 or more reports in the last 15 minutes, confirm WATER_FLOWING
                // If not, we keep the previous state (Blue indefinitely until HU-09).
                if (recentRestoredReports.length >= 2) {
                    if (zone.status !== ZoneStatus.WATER_FLOWING) {
                        this.logger.log(`Zone ${zone.name} (${zone.id}) status updated to WATER_FLOWING based on inference.`);
                        await this.zoneRepository.updateZone(zone.id, { status: ZoneStatus.WATER_FLOWING });
                    }
                }
            }
        } catch (error) {
